 ...
 * convenience methods , but they are useful when working with the
...
//    }
     ...
     */
    public static Script createMultiSigOutputScript(int threshold, List<byte[]> pubkeys) throws Exception {
        checkArgument(threshold > 0);
        checkArgument(threshold <= pubkeys.size());
        checkArgument(pubkeys.size() <= 16);  // That's the max we can represent with a single opcode.
        if (threshold < 0) throw new Exception("negative quorum");
        if (pubkeys.size() < 0) throw new Exception("negative pubkey count");
        if (threshold > pubkeys.size()) throw new Exception("quorum too big");
        if (0 == threshold && pubkeys.size() > 0)
            throw new Exception("quorum empty with non-empty pubkey list");
     ...
     */
    public static Script createP2SHOutputScript(int threshold, List<byte[]> pubkeys) throws Exception {
     ...
     * redeem script in the lexicographical sorting order.
     */
    public static Script createRedeemScript(int threshold, List<byte[]> pubkeys) throws Exception {
