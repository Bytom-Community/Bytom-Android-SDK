...
public class SegwitAddress {
    private static SegwitAddress instance = new SegwitAddress();

    private SegwitAddress() {}

    public static SegwitAddress getInstance() {
        return instance;
    }

    public Pair<Byte, byte[]> decode(String hrp, String addr) throws Exception {
        Pair<byte[], byte[]> p = Bech32.getInstance().bech32Decode(addr);
    ...
    public static String encode(byte[] hrp, byte witnessVersion, byte[] witnessProgram) throws Exception {
        ...
        return Bech32.getInstance().bech32Encode(hrp, data);
    }
